...
if (buttonExists) {
  ...
  const billedFees = [];
  const processedCPTs = new Map(); // Track processed CPT codes and their count
  
  // First count occurrences of each CPT code (ignoring modifiers for counting)
  for (const code of cptCodes) {
    const baseCode = code.split('-')[0].trim();
    processedCPTs.set(baseCode, (processedCPTs.get(baseCode) || 0) + 1);
  }
  
  console.log('CPT code occurrences:', Object.fromEntries(processedCPTs));
  
  // Process each unique CPT code
  const uniqueCPTCodes = [...new Set(cptCodes.map(code => code.split('-')[0].trim()))];
  ...
  for (let i = 0; i < uniqueCPTCodes.length; i++) {
    ...
    const originalCode = cptCodes.find(code => code.split('-')[0].trim() === baseCode);
    if (originalCode && originalCode.includes('-')) {
      modifier = originalCode.split('-')[1].trim();
    }
    
    // Get units count for this CPT code
    ...
    try {
      ...
      // Handle Confirmation Alert ("Yes" button) if it appears
      try {
      ...
      if (units > 1) {
        try {
          ...
          console.log(`✅ Updated units to ${units} for CPT ${baseCode}`);
...
app.post("/process", upload.single("file"), async (req, res) => {
  ...
  const cptCodes = [];
const cptRegex = /\b[A-Za-z]?\d{4,5}(?:-[A-Za-z0-9]{2,4})?\b/g;
let cptMatch;
