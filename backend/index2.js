...
async function processAccount(newPage, accountNumber, provider, facilityName, billingDate, diagnosisCodes,cptCodes,admitDate) {
  ...
  try {
    ...
    // ✅ CPT codes that should prevent claim creation
    const disallowedCPTs = ['99201', '99211', '99221', '99231', '99241'];

    // ✅ Check if any CPT code starts with a disallowed code
    const hasDisallowedCPT = cptCodes.some(code =>
        disallowedCPTs.some(disallowed => code.startsWith(disallowed))
    );

    if (hasDisallowedCPT) {
        const foundCodes = cptCodes.filter(code =>
            disallowedCPTs.some(disallowed => code.startsWith(disallowed))
        );

        console.warn(`⛔ Disallowed CPT codes found: ${foundCodes.join(", ")} — skipping claim creation.`);
        return { 
            success: false, 
            disallowedCPT: foundCodes,   // ✅ send back to Excel
            message: 'Claim creation skipped due to disallowed CPT code' 
        };
    }
...
// --- diagnosis code loop ---
let dxCodeNotFound = []; // ✅ only store DX codes, not full modal text
...
for (let i = 0; i < diagnosisCodes.length; i++) {
    ...
    try {
        ...
        if (hasErrorModal) {
            ...
            // ✅ Only store the DX code itself (not the full modal text)
            dxCodeNotFound.push(code);
...
app.post("/process", upload.single("file"), async (req, res) => {
    ...
    try {
        ...
        const originalHeaders = originalData[0].map(h => h?.toString().trim());
        console.log("Original Headers:", originalHeaders);
        
        // Check if "ECW #" exists in headers
        if (!originalHeaders.includes("ECW #")) {
            throw new Error("'ECW #' Column is not defined.");
        }
        
        const requiredColumns = ["ECW #", "Provider", "Hospital Location", "Billing Date"];
        for (const column of requiredColumns) {
            if (!originalHeaders.includes(column)) {
                throw new Error(`Required column '${column}' is missing from the spreadsheet.`);
            }
        }

        // BULLETPROOF COLUMN ORDERING APPROACH
        // Step 1: Remove "Result" from original headers if it exists
        const headersWithoutResult = originalHeaders.filter(header => header !== "Result");
        
        // Step 2: Add missing columns (except Result)
        const requiredExtraColumns = ["Claim Number", "Billed Fee"];
        const finalHeadersWithoutResult = [...headersWithoutResult];
        
        for (const header of requiredExtraColumns) {
            if (!finalHeadersWithoutResult.includes(header)) {
                finalHeadersWithoutResult.push(header);
            }
        }
        
        // Step 3: Add "Result" as the LAST column
        const finalHeaders = [...finalHeadersWithoutResult, "Result"];
        
 
        
        // Step 4: Create new column mapping
        const columnMap = {};
        ...
        // Step 5: Restructure data to match new header order
        const restructuredData = [finalHeaders]; // Start with headers
        
        // Create mapping from old to new positions
        const oldColumnMap = {};
        originalHeaders.forEach((header, index) => {
            oldColumnMap[header] = index;
        });
        
        // Restructure each data row
        for (let i = 1; i < originalData.length; i++) {
            const oldRow = originalData[i] || [];
            const newRow = new Array(finalHeaders.length).fill("");
            
            // Copy existing data to new positions
            finalHeaders.forEach((header, newIndex) => {
                if (oldColumnMap.hasOwnProperty(header)) {
                    const oldIndex = oldColumnMap[header];
                    newRow[newIndex] = oldRow[oldIndex] || "";
                }
                // New columns (Claim Number, Billed Fee, Result) will remain empty strings
            });
            
            restructuredData.push(newRow);
        }
        
        // Replace original data with restructured data
        originalData.length = 0; // Clear original array
        originalData.push(...restructuredData);
        ...
        for (let i = 1; i < originalData.length; i++) {
            ...
            try {
                const { success, claimNumber, billedFees, providerNotFound, facilityNotFound, patientNotFound, dxCodeNotFound, disallowedCPT } = await processAccount(
                    newPage,
                    accountNumber,
                    provider,
                    facilityName,
...
if (providerNotFound) {
    ...
    console.log(`\x1b[1;31m❌ Account ${accountNumber}: Provider Not Found - written to Excel\x1b[0m`);
    // Bold + Yellow for Result
    console.log(`\x1b[1;33mResult: ${row[columnMap["Result"]]}\x1b[0m`);
    ...
    console.log(`\x1b[1;31m❌ Account ${accountNumber}: Facility Not Found - written to Excel\x1b[0m`);
    console.log(`\x1b[1;33mResult: ${row[columnMap["Result"]]}\x1b[0m`);
    ...
    console.log(`\x1b[1;31m❌ Account ${accountNumber}: Patient Not Found - written to Excel\x1b[0m`);
    console.log(`\x1b[1;33mResult: ${row[columnMap["Result"]]}\x1b[0m`);

} else if (disallowedCPT && disallowedCPT.length > 0) {
    results.failed.push(accountNumber);
    row[columnMap["Result"]] = "Failed";
    row[columnMap["Claim Number"]] = "";
    row[columnMap["Billed Fee"]] = `Disallowed CPT: ${disallowedCPT.join(", ")}`;

    console.log(`\x1b[1;31m❌ Account ${accountNumber}: Disallowed CPT - ${disallowedCPT.join(", ")} written to Excel\x1b[0m`);
    console.log(`\x1b[1;33mResult: ${row[columnMap["Result"]]}\x1b[0m`);
}
else if (success) {
    ...
    if (Array.isArray(dxCodeNotFound) && dxCodeNotFound.length > 0) {
        if (dxCodeNotFound.length === 1) {
            messages.push(`${dxCodeNotFound[0]} is Non-Billable code`);
        } else {
            messages.push(`${dxCodeNotFound.join(", ")} are Non-Billable codes`);
        }
    }
   ...
   // Bold + Green for account success
console.log(`\x1b[1;32m✅ Account ${accountNumber}: Successfully processed\x1b[0m`);

// Bold + Yellow for Result
console.log(`\x1b[1;33mResult: ${row[columnMap["Result"]]}\x1b[0m`);

// Bold + Cyan for Claim Number
console.log(`\x1b[1;36m🧾 Claim Number: ${claimNumber}\x1b[0m`);
   ...
   // Bold + Red for failure
console.log("\x1b[1;31m%s\x1b[0m", `❌ Account ${accountNumber}: Processing failed`);

// Bold + Yellow for Result
console.log("\x1b[1;33m%s\x1b[0m", `Result: ${row[columnMap["Result"]]}`);
        ...
        // Use finalHeaders (which guarantees Result is last)
        ...
        originalData.slice(1).forEach((row) => {
            ...
            // Row data is already in correct order due to restructuring
            const rowData = row;
            ...
            headers.forEach((header, colIndex) => {
                ...
                // Result column formatting - THIS IS THE LAST COLUMN NOW
