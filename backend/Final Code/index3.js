...
const { noResults, multipleSuggestions } = await page.evaluate(() => {
  const noResultsElement = document.querySelector('#autocomplete-noresultsICD10');
  if (noResultsElement && noResultsElement.style.display !== 'none') {
    return { noResults: true, multipleSuggestions: false };
  }

  // Get only visible, non-empty suggestion items
  let items = Array.from(document.querySelectorAll('.ui-autocomplete li'))
    .filter(li => li.offsetParent !== null) // must be visible
    .map(li => li.textContent.trim())
    .filter(text => text && !/^(icd-?10|search results|no results)/i.test(text)); 
    // remove headers or junk

  console.log("📋 Suggestions seen by script:", items); // <-- add debugging log

  if (items.length === 0) return { noResults: true, multipleSuggestions: false };
  if (items.length > 1) return { noResults: false, multipleSuggestions: true };
  return { noResults: false, multipleSuggestions: false };
});

    if (noResults || multipleSuggestions) {
      console.warn(
        `❌ Skipping ${code} because ${noResults ? "no results" : "multiple suggestions"}`
      );
      diagnosisNotFoundList.push(code);
    ...
    // If exactly 1 suggestion → select it
    let selected = false;
    try {
      await input.focus();
      await new Promise(resolve => setTimeout(resolve, 200));
